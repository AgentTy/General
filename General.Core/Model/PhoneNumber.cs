...
namespace General.Model
{
	...
	*/
	...
	public class PhoneNumber
	{
		...
		{
			if (!String.IsNullOrWhiteSpace(DefaultUSAreaCode))
		...
		private void ParseNANPNumber(Int64 intNumber)
		{
			if (intNumber != -1)
			{
				...
				if (strTemp.Length == 7 && !String.IsNullOrWhiteSpace(DefaultUSAreaCode))
				{
		...
		private Int64 CleanNumber(string strNumber)
		{
			...
			else
			{
				...
				else
				{
					...
					if (StringFunctions.ContainsAfterDigits(strTemp, ",,"))
						strTemp = strTemp.Replace(",,", "^");
					if (StringFunctions.ContainsAfterDigits(strTemp, ","))
						strTemp = strTemp.Replace(",", "^");
					if (StringFunctions.ContainsAfterDigits(strTemp, ";"))
						strTemp = strTemp.Replace(";", "^");
					if (StringFunctions.ContainsAfterDigits(strTemp, "p"))
		...
		/// </summary>
		public string ToUSDialString(bool AddExtension = true)
		{
			return (ToDialString("1", AddExtension));
		}

		/// <summary>
		/// Returns a dial string from a point of origination
		/// </summary>
		public string ToDialString(PhoneNumber OriginatingPhoneNumber, bool AddExtension)
		{
			return (ToDialString(OriginatingPhoneNumber.CountryCode.ToString(), AddExtension));
		}

		/// <summary>
		/// Returns a dial string from a point of origination
		/// </summary>
		public string ToDialString(string OriginatingCountryCode, bool AddExtension)
		{
			if (StringFunctions.IsNumeric(OriginatingCountryCode))
				return (ToDialString(Convert.ToInt64(OriginatingCountryCode), AddExtension));
			else
				return (ToDialString(GetPhoneCountryCodeFromISOCountryCode(OriginatingCountryCode), AddExtension));
		}

		/// <summary>
		/// Returns a dial string from a point of origination
		/// </summary>
		public string ToDialString(Int64 OriginatingCountryCode, bool AddExtension)
		{
			string strNDD;
			string strIDD;
			XmlDocument Countries = GetCountryDatabase();
			...
			foreach (XmlNode n in nl)
			{
				if (n.Attributes["CountryCode"].Value == OriginatingCountryCode.ToString())
				{
					...
					return (ToDialString(OriginatingCountryCode, strIDD, strNDD, AddExtension));
				}
		...
		private string ToDialString(Int64 OriginatingCountryCode, string IDD, string NDD, bool AddExtension)
		{
			...
			else
			{
				...
				if (AddExtension && _intExtension.HasValue && _intExtension >= 0)
				{
					sb.Append(",,");
					sb.Append(_intExtension.Value.ToString());
				}
		...
		public ParseArgument ParseAs
		{
			...
			set
			{
				_enuParseAs = value; _intPropertiesLeftToPopulateObject--;
		...
		*/
		...
		public static string GetISOCountryCodeFromUserLocale()
		{
            ...
            if (String.IsNullOrWhiteSpace(strLanguageCodes))
