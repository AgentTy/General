...
namespace General.Mail {
	...
	public class MailTools 
	{
        ...
        public static bool SendEmail(EmailAddress FromEmail, System.Collections.ArrayList ToEmail, string Subject, string Body, bool IsHtml, System.Collections.ArrayList Attachments) 
		{
            ...
            if (Environment.Current.AmIDev())
            {
                ...
                if (ToEmail.Count > 1)
                {
                    ...
                    {
                        if (!String.IsNullOrWhiteSpace(objEmail.ToString()))
            ...
            if (SendWithBulkMailServer && ToEmail.Count > 1 && !String.IsNullOrWhiteSpace(MailServerBulkEmail))
                objMailServer = GetMailServer(MailServerTypes.Bulk);
            else if (!Environment.Current.AmIDev() && !String.IsNullOrWhiteSpace(MailServerDebug) && ToEmail.Count == 1 && (((EmailAddress)ToEmail[0]) == new EmailAddress(Debugging.Report.ErrorEmailTo) || ((EmailAddress)ToEmail[0]) == new EmailAddress(Debugging.Report.DebugEmailTo)))
        ...
        public static SmtpClient GetMailServer(MailServerTypes MailServerType)
        {
            ...
            if (MailServerType == MailServerTypes.Bulk && !String.IsNullOrWhiteSpace(MailServerBulkEmail))
            {
                //Use separate mail server for bulk email
                strMailServer = MailServerBulkEmail;
                if (!String.IsNullOrWhiteSpace(MailServerBulkEmail_Port))
                    intPort = int.Parse(MailServerBulkEmail_Port);
                if (!String.IsNullOrWhiteSpace(MailServerBulkEmail_UserName))
                    strUserName = MailServerBulkEmail_UserName;
                if (!String.IsNullOrWhiteSpace(MailServerBulkEmail_Password))
                    strPassword = MailServerBulkEmail_Password;
            }
            else if (MailServerType == MailServerTypes.Debug && !String.IsNullOrWhiteSpace(MailServerDebug))
            {
                //Use separate mail server for debugging emails and error reporting
                strMailServer = MailServerDebug;
                if (!String.IsNullOrWhiteSpace(MailServerDebug_Port))
                    intPort = int.Parse(MailServerDebug_Port);
                if (!String.IsNullOrWhiteSpace(MailServerDebug_UserName))
                    strUserName = MailServerDebug_UserName;
                if (!String.IsNullOrWhiteSpace(MailServerDebug_Password))
            ...
            else
            {
                if (Environment.Current.AmILive())
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(MailServerLive_Port))
                        intPort = int.Parse(MailServerLive_Port);
                    if (!String.IsNullOrWhiteSpace(MailServerLive_UserName))
                        strUserName = MailServerLive_UserName;
                    if (!String.IsNullOrWhiteSpace(MailServerLive_Password))
                        strPassword = MailServerLive_Password;
                }
                else if (Environment.Current.AmIStage())
                {
                    strMailServer = MailServerStage;
                    if (!String.IsNullOrWhiteSpace(MailServerStage_Port))
                        intPort = int.Parse(MailServerStage_Port);
                    if (!String.IsNullOrWhiteSpace(MailServerStage_UserName))
                        strUserName = MailServerStage_UserName;
                    if (!String.IsNullOrWhiteSpace(MailServerStage_Password))
                        strPassword = MailServerStage_Password;
                }
                else if (Environment.Current.AmIQA())
                {
                    strMailServer = MailServerQA;
                    if (!String.IsNullOrWhiteSpace(MailServerQA_Port))
                        intPort = int.Parse(MailServerQA_Port);
                    if (!String.IsNullOrWhiteSpace(MailServerQA_UserName))
                        strUserName = MailServerQA_UserName;
                    if (!String.IsNullOrWhiteSpace(MailServerQA_Password))
                        strPassword = MailServerQA_Password;
                }
                else
                {
                    strMailServer = MailServerDev;
                    if (!String.IsNullOrWhiteSpace(MailServerDev_Port))
                        intPort = int.Parse(MailServerDev_Port);
                    if (!String.IsNullOrWhiteSpace(MailServerDev_UserName))
                        strUserName = MailServerDev_UserName;
                    if (!String.IsNullOrWhiteSpace(MailServerDev_Password))
                        strPassword = MailServerDev_Password;
                }
            }

            if (String.IsNullOrWhiteSpace(strMailServer))
            {
                strMailServer = MailServerLive;
                if (!String.IsNullOrWhiteSpace(MailServerLive_Port))
                    intPort = int.Parse(MailServerLive_Port);
