    ...
    public class WordpressFunctions
    {
        ...
        */
        ...
        public static string wpautop(string pee, bool br)
        {
            ...
            if (pee.Contains("<pre"))
            {
                //Just bypass if there is any preformatted text
                return pee;
                ...
                foreach (string pee_part in pee_parts)
                {
                    ...
                    if (start < 0)
                    {
                        //This was the original line, Ty changed it to try to fix a bug where </pre> was removed... didn't work???
                        //pee += pee_part;

                        if (pee_part.Replace("\r", "").Replace("\n", "").Trim().Length == 0)
                            pee += "</pre>";
                        else
                            pee += pee_part;
                        continue;
            ...
            {
                pee = Regex.Replace(pee, @"\s*<param([^>]*)>\s*", "<param1>"); // no pee inside object/embed
                pee = Regex.Replace(pee, @"\s*</embed>\s*", "</embed>");
            }
            ...
            pee = Regex.Replace(pee, @"<p>\s*</p>", ""); // under certain strange conditions it could create a P of entirely whitespace
            pee = Regex.Replace(pee, @"<p>([^<]+)</(div|address|form)>", "<p>$1</p></$2>");
            pee = Regex.Replace(pee, @"<p>\s*(</?" + allblocks + @"[^>]*>)\s*</p>", "$1"); // don't pee all over a tag
            pee = Regex.Replace(pee, @"<p>(<li.+?)</p>", "$1"); // problem with nested lists
            pee = Regex.Replace(pee, @"<p><blockquote([^>]*)>", "<blockquote$1><p>");
