...
namespace General.Utilities.Text
{
  ...
  public class BoyerMoore
  {
    ...
    public static IEnumerable<int> BoyerMooreSearch(string text, string search)
    {
        BoyerMoore objSearch = new BoyerMoore(search);
        return objSearch.BoyerMooreMatch(text);
    }

    public static int? BoyerMooreFirstIndex(string text, string search)
    {
        BoyerMoore objSearch = new BoyerMoore(search);
        IEnumerable<int> objResult = objSearch.BoyerMooreMatch(text);
        if (objResult != null)
            return objResult.GetEnumerator().Current;
        else 
            return null;
    }

    public static string BoyerMooreReplace(string text, string search, string replace)
    {
        IEnumerable<int> objMatches = BoyerMooreSearch(text, search);
        foreach (int intIndex in objMatches)
        {
            text = text.Remove(intIndex, search.Length);
            text = text.Insert(intIndex, replace);
        }
        return text;
    }

    public static string BoyerMooreReplaceOnce(string text, string search, string replace)
    {
        IEnumerable<int> objMatches = BoyerMooreSearch(text, search);
        foreach (int intIndex in objMatches)
        {
            text = text.Remove(intIndex, search.Length);
            text = text.Insert(intIndex, replace);
            return text; //Return after the first iteration
        }
        return text;
    }
    #endregion
